...
async fn openai_route_strips_prefix_sets_auth_and_applies_model_mapping() {
    ...
    let (mock_addr, mock_handle) = spawn_capture_mock("/v1/chat/completions", Arc::clone(&captured)).await;
...
async fn anthropic_route_strips_prefix_sets_anthropic_headers_and_filters_protocol() {
    ...
    let (openai_addr, openai_handle) = spawn_capture_mock("/v1/messages", Arc::clone(&openai_hits)).await;
    let (anthropic_addr, anthropic_handle) = spawn_capture_mock("/v1/messages", Arc::clone(&anthropic_hits)).await;
    let config = TestConfigBuilder::new()
        .backend(TestBackend::openai("openai-backend", openai_addr).with_models(&["mock-model"]))
        .backend(TestBackend::anthropic("anthropic-backend", anthropic_addr).with_models(&["mock-model"]))
        .build();
    let (proxy_addr, proxy_handle) = spawn_proxy(config).await;
    let client = Client::new();
...
async fn model_list_routes_return_protocol_specific_shapes() {
    let (mock_addr, mock_handle) = spawn_capture_mock("/unused", Arc::new(Mutex::new(Vec::new()))).await;
    let config = TestConfigBuilder::new()
        .backend(TestBackend::openai("openai-backend", mock_addr).with_models(&["model-a", "model-b"]))
        .build();
    let (proxy_addr, proxy_handle) = spawn_proxy(config).await;
    let client = Client::new();
    ...
    let anthropic_json: serde_json::Value = serde_json::from_str(&anthropic_text).expect("anthropic json");
    assert!(anthropic_json["models"].as_array().expect("anthropic models").len() >= 2);
...
async fn capture_handler(
    ...
    captured.lock().expect("captured requests").push(CapturedRequest {
